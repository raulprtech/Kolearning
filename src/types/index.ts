...
export const EvaluateOpenAnswerInputSchema = z.object({
  ...
  correctAnswer: z
    .string()
    .describe('The ground truth correct answer to the question.'),
  userAnswer: z.string().describe("The user's submitted answer."),
});
export type EvaluateOpenAnswerInput = z.infer<
  typeof EvaluateOpenAnswerInputSchema
>;
...
export type EvaluateOpenAnswerOutput = z.infer<
  typeof EvaluateOpenAnswerOutputSchema
>;
...
export type GenerateDeckFromTextInput = z.infer<
  typeof GenerateDeckFromTextInputSchema
>;

const KnowledgeAtomSchema = z.object({
  atomo_id: z
    .string()
    .describe("A unique ID for the atom within this material (e.g., 'ALG023')."),
  material_id: z
    .string()
    .describe("The ID for the entire deck (e.g., 'MAT001')."),
  concepto: z
    .string()
    .describe(
      'The core concept, term, or question (the "front" of the flashcard).'
    ),
  descripcion: z
    .string()
    .describe(
      'The definition, explanation, or answer (the "back" of the flashcard). Supports Markdown and LaTeX.'
    ),
  atomos_padre: z
    .array(z.string())
    .describe(
      "An array of 'atomo_id's that are prerequisites for this atom. Empty if none."
    ),
  formatos_presentacion: z
    .array(z.string())
    .describe(
      "Suitable learning formats (e.g., 'Identificación', 'Ejemplificación')."
    ),
  dificultad_inicial: z
    .string()
    .describe(
      "Initial difficulty level (e.g., 'Fundamental', 'Intermedio', 'Avanzado')."
    ),
});
...
export const GenerateDeckFromTextOutputSchema = z.object({
  title: z
    .string()
    .describe('A concise and relevant title for the generated flashcard deck.'),
  description: z
    .string()
    .describe('A brief, one-sentence description of the flashcard deck.'),
  material_id: z
    .string()
    .describe("A unique ID for this entire deck (e.g., 'MAT001')."),
  flashcards: z
    .array(KnowledgeAtomSchema)
    .describe(
      'An array of all relevant knowledge atoms (flashcards) based on the key concepts in the notes. Do not limit the number of cards.'
    ),
});
export type GenerateDeckFromTextOutput = z.infer<
  typeof GenerateDeckFromTextOutputSchema
>;
...
export type GenerateOptionsForQuestionInput = z.infer<
  typeof GenerateOptionsForQuestionInputSchema
>;
...
export type GenerateOptionsForQuestionOutput = z.infer<
  typeof GenerateOptionsForQuestionOutputSchema
>;
...
export type RefineProjectDetailsInput = z.infer<
  typeof RefineProjectDetailsInputSchema
>;

export const RefineProjectDetailsOutputSchema = z.object({
  title: z
    .string()
    .describe('A concise, descriptive, and engaging title for the project in Spanish.'),
  description: z
    .string()
    .describe('A one or two-sentence summary of the project in Spanish.'),
  category: z
    .string()
    .describe(
      'A relevant, single-word category for the project in Spanish (e.g., "Programación", "Biología", "Historia").'
    ),
});
export type RefineProjectDetailsOutput = z.infer<
  typeof RefineProjectDetailsOutputSchema
>;
...
export const GenerateStudyPlanInputSchema = z.object({
  ...
  objective: z
    .string()
    .describe('The user\'s learning goal (e.g., "pass an exam", "review for an interview").'),
  timeLimit: z
    .string()
    .describe('The time frame the user has to study (e.g., "2 weeks", "1 month").'),
  masteryLevel: z
    .string()
    .describe('The user\'s self-assessed mastery level (e.g., "Beginner", "Intermediate").'),
  flashcards: z
    .array(StudyPlanFlashcardSchema)
    .describe('The list of knowledge atoms (flashcards) for the project.'),
  cognitiveProfile: z
    .array(z.string())
    .optional()
    .describe("User's preferred learning styles (e.g., 'Visual', 'Practical Examples')."),
  learningChallenge: z
    .string()
    .optional()
    .describe("User's biggest self-reported learning challenge."),
});
...
export const GenerateStudyPlanOutputSchema = z.object({
  plan: z
    .array(
      z.object({
        topic: z
          .string()
          .describe('The specific topic or concept to be covered in this session.'),
        sessionType: z
          .string()
          .describe(
            "The recommended session type (e.g., 'Calibración Inicial', 'Refuerzo de Dominio', 'Consulta con Koli')."
          ),
      })
    )
  ...
  justification: z
    .string()
    .describe('A brief, encouraging explanation of why the plan is structured this way.'),
  expectedProgress: z
    .string()
    .describe(
      "A brief explanation of the expected progress after each major section or day of the plan."
    ),
  category: z
    .string()
    .describe(
