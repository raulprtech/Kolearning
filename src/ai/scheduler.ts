...
export interface CronJobConfig {
    id: string;
    schedule: string; // ISO 8601, interval string, or cron expression
    flowName: string;
    payload?: any;
    description?: string;
    enabled?: boolean;
...
export class Scheduler {
    private jobs: Map<string, { config: CronJobConfig; cron: Cron }> = new Map();
    ...
    private loadJobs() {
        try {
            if (fs.existsSync(JOBS_FILE)) {
                ...
                const configs: CronJobConfig[] = JSON.parse(data);
    ...
    public scheduleJob(config: CronJobConfig) {
        if (this.jobs.has(config.id)) {
            this.stopJob(config.id);
        }

        if (config.enabled === false) {
            this.jobs.set(config.id, { config, cron: null as any });
            return;
        }
        ...
        const cron = new Cron(config.schedule, async () => {
            ...
            try {
                ...
                const flow = (allFlows as any)[config.flowName];
                if (!flow) {
    ...
    public addJob(config: CronJobConfig) {
        this.scheduleJob({ ...config, enabled: config.enabled ?? true });
        this.saveJobs();
    }
    ...
    public listJobs(): CronJobConfig[] {
        return Array.from(this.jobs.values()).map(j => j.config);
    }

    public async runJobNow(id: string) {
        const job = this.jobs.get(id);
        if (!job) throw new Error(`Job ${id} not found`);

        console.log(`[Scheduler] Manually triggering job: ${id}`);
        const allFlows = await import('./flows');
        const flow = (allFlows as any)[job.config.flowName];
        if (!flow) throw new Error(`Flow ${job.config.flowName} not found`);
