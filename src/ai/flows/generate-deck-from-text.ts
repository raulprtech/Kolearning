...
const GenerateDeckFromTextInputSchema = z.object({
  studyNotes: z.string().describe('The study notes to generate a flashcard deck from.'),
});
...
const GenerateDeckFromTextOutputSchema = z.object({
  ...
  flashcards: z.array(
    z.object({
      question: z.string().describe('The flashcard question (supports Markdown). Should be a clear, answerable question based on the notes.'),
      answer: z.string().describe('The flashcard answer (supports Markdown). Should be a concise and accurate answer to the question.'),
    })
...
Then, create between 5 and 15 flashcards, focusing on the most important concepts, definitions, and key facts in the text. Each flashcard must have a clear question and a concise answer. Use Markdown for formatting if it helps clarity (e.g., for code or lists).
