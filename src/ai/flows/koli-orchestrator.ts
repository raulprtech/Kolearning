...
export const koliOrchestrator = ai.defineFlow(
    ...
    async (input, streamingCallback) => {
        ...
        const assistantName = input.assistantConfig?.name || 'Koli';
        ...
        const response = await ai.generate({
            ...
            tools: [searchArticlesTool, createProjectTool, searchDataBoxTool, startStudySessionTool],
            onChunk: (chunk) => {
        ...
        console.log(`[koliOrchestrator] AI Response: ${response.text?.substring(0, 50)}...`);
                    ...
                    if (content.toolResponse) {
                        ...
                        for (let i = msgIdx - 1; i >= 0; i--) {
                            ...
                            if (prevMsg.role === 'model') {
                                const toolReq = prevMsg.content.find(c =>
                                    c.toolRequest &&
                                    (c.toolRequest.name === name || c.toolRequest.ref === ref)
                                );
                                if (toolReq?.toolRequest) {
