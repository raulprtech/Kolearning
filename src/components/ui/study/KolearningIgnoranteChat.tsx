...
type ChatMessage = {
    role: 'user' | 'kolearning';
    content: string;
};

type KolearningIgnoranteChatProps = {
    /** The concept the user should teach */
    concept: string;
    /** The expected correct explanation */
    expectedExplanation: string;
    /** Kolearning's initial "ignorant" question */
 ...
 * Kolearning pretends to be ignorant about a topic and the user must teach it.
 * Kolearning asks probing questions to test whether the user truly understands.
 * After 3-5 exchanges, Kolearning evaluates if the user demonstrated mastery.
 */
export const KolearningIgnoranteChat = ({
    concept,
    expectedExplanation,
    initialQuestion,
    onComplete,
    onAnswerSelect,
}: KolearningIgnoranteChatProps) => {
    ...
    useEffect(() => {
        const kolearningIntro: ChatMessage = {
            role: 'kolearning',
    ...
    const handleSend = async () => {
        ...
        const fullConversation = updatedMessages
            .map(m => `${m.role === 'user' ? 'Usuario' : 'Kolearning'}: ${m.content}`)
            .join('\n');
        onAnswerSelect(fullConversation);

        try {
            const { kolearningIgnoranteRespond } = await import('@/ai/flows/kolearning-ignorante-chat');

            const shouldEvaluate = newExchangeCount >= 3;

            const result = await kolearningIgnoranteRespond({
                concept,
                expectedExplanation,
            ...
            const kolearningResponse: ChatMessage = { role: 'kolearning', content: result.response };
            setMessages(prev => [...prev, kolearningResponse]);
            ...
            console.error('Error in Kolearning Ignorante chat:', error);
...
export default KolearningIgnoranteChat;
