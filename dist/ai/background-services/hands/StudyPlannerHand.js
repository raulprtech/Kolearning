 ...
 */
export async function studyPlannerHand(userId) {
    ...
    if (cognitiveLayer.proactive_insights.length > 3)
        cognitiveLayer.proactive_insights.shift();
    await supabase.from('profiles').update({
        additional_info: JSON.stringify(Object.assign(Object.assign({}, info), { cognitive_layer: cognitiveLayer }))
    }).eq('id', userId);
