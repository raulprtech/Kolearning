...
export const convertPaperFromDB = (paperRow) => ({
    id: paperRow.id,
...
// Convert database types to frontend types without additional async calls
export const convertProjectFromDBSync = (projectRow) => {
    ...
    const sessions = (projectRow.sessions || []).map((sessionData) => {
        // Get the IDs of atoms linked to this session
        const linkedAtomIds = (sessionData.session_atoms || []).map((sa) => sa.atom_id);
        ...
        const sessionAtoms = atoms.filter((atom) => linkedAtomIds.includes(atom.id));
        return {
            ...
            phase: sessionData.phase || undefined,
...
export const convertProjectFromDB = async (projectRow, includeRelated = true) => {
    ...
    if (includeRelated) {
        ...
        atoms = (atomsData === null || atomsData === void 0 ? void 0 : atomsData.map(convertAtomFromDB)) || [];
        // Fetch sources
        const { data: sourcesData } = await supabase
            ...
            .eq('project_id', projectRow.id)
            .order('created_at', { ascending: true });
        sources = (sourcesData === null || sourcesData === void 0 ? void 0 : sourcesData.map(convertSourceFromDB)) || [];
        ...
        if (sessionsData) {
            sessions = sessionsData.map((sessionData) => {
                const linkedAtomIds = (sessionData.session_atoms || []).map((sa) => sa.atom_id);
                const sessionAtoms = atoms.filter((atom) => linkedAtomIds.includes(atom.id));
                return {
                    session: sessionData.session_number,
        ...
        const { data: learningPathData } = await supabase
            ...
            .eq('project_id', projectRow.id)
            .order('session_number', { ascending: true });
        learningPath = (learningPathData === null || learningPathData === void 0 ? void 0 : learningPathData.map(convertLearningPathItemFromDB)) || [];
    }
    return {
        id: projectRow.id,
        title: projectRow.title,
...
export const convertAtomFromDB = (atomRow) => {
    ...
    return {
        ...
        difficulty: parseFloat(atomRow.difficulty),
...
export const convertSourceFromDB = (sourceRow) => ({
    id: sourceRow.id,
    name: sourceRow.name,
    type: sourceRow.type,
...
export class ProjectDatabase {
    ...
    async createProject(userId, project, log) {
        ...
        if (project.sources.length > 0) {
            ...
            const { error: sourcesError } = await this.supabase
                .from('sources')
                .insert(project.sources.map(source => ({
                project_id: projectId,
                name: source.name,
                type: source.type,
                content: source.content,
        ...
        if (project.learningPath.length > 0) {
            ...
            const { error: learningPathError } = await this.supabase
                .from('learning_path_items')
                .insert(project.learningPath.map(item => ({
                project_id: projectId,
    ...
    async updateLearningPathAndSessions(projectId, learningPath, sessions) {
        ...
        const { error: deleteLpError } = await this.supabase
            ...
            .eq('project_id', projectId);
        if (deleteLpError)
            throw new Error(`Failed to delete old learning path: ${deleteLpError.message}`);
        ...
        if (deleteSessionsError)
            throw new Error(`Failed to delete old sessions: ${deleteSessionsError.message}`);
        ...
        if (learningPath.length > 0) {
            ...
            if (insertLpError)
                throw new Error(`Failed to insert new learning path: ${insertLpError.message}`);
        }
        ...
        if (sessions.length > 0) {
            ...
            if (insertSessionsError)
                throw new Error(`Failed to insert new sessions: ${insertSessionsError.message}`);
    ...
    async updateAtom(projectId, atom) {
        if (atom.id) {
    ...
    async addAtoms(projectId, atoms) {
        if (atoms.length === 0)
            return;
        const { error } = await this.supabase
            .from('atoms')
            .insert(atoms.map(atom => ({
            project_id: projectId,
            question: atom.question,
            answer: atom.answer,
            type: atom.type || 'text_card',
    ...
    async updateAtoms(projectId, atoms) {
        if (atoms.length === 0)
            return;
    ...
    async addSessions(projectId, sessions) {
        if (sessions.length === 0)
            return;
        const { error } = await this.supabase
            .from('sessions')
    ...
    async createPaper(userId, paper) {
    ...
    async updatePaper(paperId, updates) {
        ...
        if (updates.authors !== undefined)
            dbUpdates.authors = updates.authors;
        if (updates.year !== undefined)
            dbUpdates.year = updates.year;
        if (updates.doi !== undefined)
            dbUpdates.doi = updates.doi;
        if (updates.journalConference !== undefined)
            dbUpdates.journal_conference = updates.journalConference;
        if (updates.url !== undefined)
            dbUpdates.url = updates.url;
        if (updates.pdfStatus !== undefined)
            dbUpdates.pdf_status = updates.pdfStatus;
        if (updates.importSource !== undefined)
            dbUpdates.import_source = updates.importSource;
        if (updates.status !== undefined)
            dbUpdates.status = updates.status;
        if (updates.processingPercentage !== undefined)
            dbUpdates.processing_percentage = updates.processingPercentage;
        if (updates.fieldOfKnowledge !== undefined)
            dbUpdates.field_of_knowledge = updates.fieldOfKnowledge;
        if (updates.difficultyLevel !== undefined)
            dbUpdates.difficulty_level = updates.difficultyLevel;
        if (updates.paperType !== undefined)
            dbUpdates.paper_type = updates.paperType;
        if (updates.tags !== undefined)
            dbUpdates.tags = updates.tags;
        if (updates.readingStatus !== undefined)
            dbUpdates.reading_status = updates.readingStatus;
